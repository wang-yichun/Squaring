 ...
 *  1.8 Layer互斥管理,模态Layer: mutex_layers, modal
 *   todo: 1.6 场景动画（全局）
 *   todo: 1.7 动态层序
 *   todo: 1.8 信息传递
 */
...
// 6 remove 层垃圾
// 7 debug
    logLevel: {1: true, 2: true, 3: true, 4: true, 5: true, 6: true, 7: true},
    ...
    addLayersForScene: function (para) {
        ...
        for (var group_id in scene_info.layers) { // 该层拥有的所有组
            ...
            for (var layer_id in group_info) {
                ...
                cc.log("layer_info.ccbi >>" + layer_info.ccbi);
                cc.log("Switcher.currentScene >>" + Switcher.currentScene);
...
//                var layer_node = cc.BuilderReader.load(window[layer_info.ccbi], Switcher.currentScene);
    ...
    checkSwitchingGroups_AfterIn: function () {
        ...
        if (Switcher.isCurrentSwitchingGroupsHaveTrue() == false) {
            ...
            Switcher.unfreezeCurrentLayers();
        }
    },
    ...
    frozenCurrentLayers: function () {
        for (var group_id in Switcher.currentLayerIDs) {
            var layer_id = Switcher.currentLayerIDs[group_id];
            if (layer_id == null) continue;
            var layer_node = Switcher.currentScene.switcher_para.layers[layer_id];
            Switcher.setLayerMenuItemEnabled(layer_node, false);
        }
    },

    unfreezeCurrentLayers: function () {
        for (var group_id in Switcher.currentLayerIDs) {
            var layer_id = Switcher.currentLayerIDs[group_id];
            if (layer_id == null) continue;
            var layer_node = Switcher.currentScene.switcher_para.layers[layer_id];

            if (layer_node == null) continue;

            var layer_info = Switcher.sceneInfo[Switcher.currentScene.switcher_para.scene_id].layers[group_id][layer_id];
            if (layer_info.modal) {
                Switcher.frozenCurrentLayers();
                Switcher.setLayerMenuItemEnabled(layer_node, true);
                break;
            }
    ...
    handle_mutex: function () {

        for (var group_id in Switcher.currentLayerIDs) {
            var layer_id = Switcher.currentLayerIDs[group_id];
            if (layer_id == null) continue;

            if (Switcher.sceneInfo[Switcher.currentScene.switcher_para.scene_id].layers[group_id] == null) continue;
            var layer_info = Switcher.sceneInfo[Switcher.currentScene.switcher_para.scene_id].layers[group_id][layer_id];
            if (layer_info == null) continue;

            var mutex_layers_vector = layer_info.mutex_layers;
            if (mutex_layers_vector) {
                for (var idx in mutex_layers_vector) {
                    var it = mutex_layers_vector[idx];

                    if (it.group_id != null && it.layer_id == null) {
                        for (var layer_id_in_group in Switcher.sceneInfo[Switcher.currentScene.switcher_para.scene_id].layers[it.group_id]) {
                            var layer_node = Switcher.currentScene.switcher_para.layers[layer_id_in_group];
                            Switcher.setLayerMenuItemEnabled(layer_node, false);
                        }
                    } else if (it.layer_id != null) {
                        if (it.group_id != null) {
                            Switcher.log(1, 'group_id is unnecessary, please check out SwitcherConfig. layerID:' + it.layer_id);
                        }
                        var layer_node = Switcher.currentScene.switcher_para.layers[it.layer_id];
                        Switcher.setLayerMenuItemEnabled(layer_node, false);
                    }
                }
            }
