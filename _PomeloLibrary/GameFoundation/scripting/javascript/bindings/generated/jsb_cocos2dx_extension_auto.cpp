...
JSBool js_cocos2dx_extension_CCScrollView_stopDeaccelerateScrolling(JSContext *cx, uint32_t argc, jsval *vp)
{
    JSObject *obj = JS_THIS_OBJECT(cx, vp);
    js_proxy_t *proxy = jsb_get_js_proxy(obj);
    cocos2d::extension::CCScrollView* cobj = (cocos2d::extension::CCScrollView *)(proxy ? proxy->ptr : NULL);
    JSB_PRECONDITION2( cobj, cx, JS_FALSE, "Invalid Native Object");
    if (argc == 0) {
        bool ret = cobj->stopDeaccelerateScrolling();
        jsval jsret;
        jsret = BOOLEAN_TO_JSVAL(ret);
        JS_SET_RVAL(cx, vp, jsret);
        return JS_TRUE;
...
void js_register_cocos2dx_extension_CCScrollView(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
        ...
        JS_FN("stopDeaccelerateScrolling", js_cocos2dx_extension_CCScrollView_stopDeaccelerateScrolling, 0, JSPROP_PERMANENT | JSPROP_ENUMERATE),
...
JSBool js_cocos2dx_extension_CCTableView_deaccelerateScrollingEnded(JSContext *cx, uint32_t argc, jsval *vp)
{
    jsval *argv = JS_ARGV(cx, vp);
    JSBool ok = JS_TRUE;
    JSObject *obj = JS_THIS_OBJECT(cx, vp);
    ...
    if (argc == 1) {
        ...
        JSB_PRECONDITION2(ok, cx, JS_FALSE, "Error processing arguments");
        cobj->deaccelerateScrollingEnded(arg0);
        JS_SET_RVAL(cx, vp, JSVAL_VOID);
        return JS_TRUE;
    }
...
void js_register_cocos2dx_extension_CCTableView(JSContext *cx, JSObject *global) {
	...
	static JSFunctionSpec funcs[] = {
        ...
        JS_FN("deaccelerateScrollingEnded", js_cocos2dx_extension_CCTableView_deaccelerateScrollingEnded, 1, JSPROP_PERMANENT | JSPROP_ENUMERATE),
