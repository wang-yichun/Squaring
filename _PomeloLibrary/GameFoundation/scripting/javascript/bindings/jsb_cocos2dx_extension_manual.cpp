...
public:
    ...
    virtual void deaccelerateScrollingEnded(CCScrollView* view)
    {
        js_proxy_t * p = jsb_get_native_proxy(view);
        if (!p) return;
        
        jsval arg = OBJECT_TO_JSVAL(p->obj);
        ScriptingCore::getInstance()->executeFunctionWithOwner(OBJECT_TO_JSVAL(m_pJSDelegate), "deaccelerateScrollingEnded", 1, &arg, NULL);
    }
...
public:
    ...
    virtual void deaccelerateScrollingEnded(CCScrollView* view)
    {
        callJSDelegate(view, "deaccelerateScrollingEnded");
    }
