...
+ (BOOL)boolForKey:(NSString *)defaultName withDefaultValue:(BOOL)def {
    @synchronized(self) {
        ...
        val = [FBEncryptorAES_ori decryptBase64String:val
...
+ (void)setBool:(BOOL)value forKey:(NSString *)defaultName {
    @synchronized(self) {
        ...
        val = [FBEncryptorAES_ori encryptBase64String:val
                                        keyString:[ObscuredNSUserDefaults key]
                                    separateLines:NO];
...
+ (NSString*)stringForKey:(NSString *)defaultName withDefaultValue:(NSString*)def {
    @synchronized(self) {
        ...
        return [FBEncryptorAES_ori decryptBase64String:val
                                         keyString:[ObscuredNSUserDefaults key]];
    }
}
...
+ (void)setString:(NSString*)value forKey:(NSString *)defaultName {
    @synchronized(self) {
        NSString* val = [FBEncryptorAES_ori encryptBase64String:value
                                        keyString:[ObscuredNSUserDefaults key]
                                    separateLines:NO];
        [[NSUserDefaults standardUserDefaults] setValue:val forKey:defaultName];
        [[NSUserDefaults standardUserDefaults] synchronize];
