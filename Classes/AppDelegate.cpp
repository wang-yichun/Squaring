...
//    } else {
        searchPath.push_back("Assets/HDR");
        pDirector->setContentScaleFactor(4.0);
        int x = 320 * screenSize.width / screenSize.height;
        designSize = CCSizeMake(x, 320);
//    }
}
...
//    sc->addRegisterCallback(register_all_kompex_binding);
    ...
    CPPBridge::getInstance()->initBridge();
    
    //    JSBHelper::AddSelector("helloCpp", callfuncND_selector(CPPBridge::helloCpp), CPPBridge::getInstance());
